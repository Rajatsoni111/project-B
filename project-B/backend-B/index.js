...
app.post('/create', async (req, res) => {
    ...
    try {
        ...
        const user = await userSchema.create({ username, password: hash, email, image: defaultImage });
        if (!user) {
            throw new Error;
        }
        const token = jwt.sign({ id: user._id }, JWT_SECRET, { expiresIn: '7d' });
...
app.post('/createblog', verify, upload.single("image"), async (req, res) => {
    ...
    try {
        ...
        const blog = await blogSchema.create({ title, caption, createdBy: userr._id, tags, image: req.file.filename });
        if (!blog) {
            throw new Error;
        }
...
app.get('/searchblogs', verify, async function (req, res) {
    try {
        ...
        const blogs = await blogSchema.find({ $or: [{ title: query }, { caption: query }] });
        // let newUsers = [];
